 ...
 */
class MongoDb extends \chaos\Source
{
    /**
     ...
     */
    protected $_classes = [
        'entity'       => 'chaos\model\Entity',
        'relationship' => 'chaos\model\Relationship',
        'set'          => 'chaos\Collection',
        'result'       => 'chaos\source\mongo\Result',
        'schema'       => 'chaos\source\mongo\Schema',
        'exporter'     => 'chaos\source\mongo\Exporter',
        'server'       => 'Mongo'
    ];
     ...
     */
    public function describe($collection, $fields = [], $meta = [])
    {
    ...
    public function create($query, $options [])
    {
        ...
        $defaults = ['safe' => $_config['safe'], 'fsync' => false];
        $options += $defaults;
        $this->_checkConnection();
        ...
        return $this->_filter(__METHOD__, $params, function($self, $params) use ($_config, $_exp) {
            ...
            $args    = $query->export($self, ['keys' => ['source', 'data']]);
            $data    = $_exp::get('create', $args['data']);
            $source  = $args['source'];

            if ($source === "{$_config['gridPrefix']}.files" && isset($data['create']['file'])) {
                $result = ['ok' => true];
                $data['create']['_id'] = $self->invokeMethod('_saveFile', [$data['create']]);
            } else {
    ...
    public function read($query, $options = [])
    {
        ...
        return $this->_filter(__METHOD__, $params, function($self, $params) use ($_config) {
            ...
            if ($group = $args['group']) {
                $result = $self->invokeMethod('_group', [$group, $args, $options]);
            ...
            $result = $self->invokeMethod('_instance', ['result', compact('resource')]);
    ...
    public function update($query, $options = [])
    {
        ...
        return $this->_filter(__METHOD__, $params, function($self, $params) use ($_config, $_exp) {
            $options = $params['options'];
            $query  = $params['query'];
            $args   = $query->export($self, ['keys' => ['conditions', 'source', 'data']]);
            $source = $args['source'];
            $data   = $args['data'];
            ...
            if ($source === "{$_config['gridPrefix']}.files" && isset($data['update']['file'])) {
                $args['data']['_id'] = $self->invokeMethod('_saveFile', [$data['update']]);
            }
    ...
    public function delete($query, $options = [])
    {
        $this->_checkConnection();
        $_config = $this->_config;
        $defaults = ['justOne' => false, 'safe' => $_config['safe'], 'fsync' => false];
        ...
        return $this->_filter(__METHOD__, $params, function($self, $params) use ($_config) {
            $query = $params['query'];
            $options = $params['options'];
            $args = $query->export($self, ['keys' => ['source', 'conditions']]);
            $source = $args['source'];
            $conditions = $args['conditions'];

            if ($source === "{$_config['gridPrefix']}.files") {
                $result = $self->invokeMethod('_deleteFile', [$conditions]);
            } else {
     ...
     */
    public function calculation($type, $query, $options = [])
    {
     ...
     * @return array
     */
    public function relationship($class, $type, $name, $config = [])
    {
    ...
    protected function _checkConnection()
    {
        if (!$this->_isConnected && !$this->connect()) {
            throw new SourceException("Could not connect to the database.");
        }
    }
